...
extractor: PDFPageExtractor = create_pdf_page_extractor(
  ...
  model_dir_path="models", # Relative to the project root
)
with MarkDownWriter("output/book.md", "images", "utf-8") as md:
  for block in extractor.extract(pdf="inputs/book.pdf"):
    md.write(block)
```
...
analyse(
  ...
  pdf_path="inputs/book.pdf", # Relative to the project root
  analysing_dir_path="work/analysing/book", # analysis working directory
  output_dir_path="work/output/book", # structured analysis output
)
```
...
generate_epub_file(
  ...
  epub_file_path="dist/book.epub", # Relative to the project root
)
```
