...
class _Sequence:
  ...
  def _iter_line_ids(self, line: Element) -> Generator[int, None, None]:
    ...
    if len(ids) == 1 and ids[0].isdigit():
      id_begin = int(ids[0])
      id_end = id_begin
    elif len(ids) == 2 and ids[0].isdigit() and ids[1].isdigit():
      id_begin = int(ids[0])
      id_end = int(ids[1])
    else:
      matched_ids = re.findall(r"\d+", raw_ids)
      if len(matched_ids) == 0:
        return
      if len(matched_ids) == 1:
        id_begin = int(matched_ids[0])
        id_end = id_begin
      else:
        id_begin = int(matched_ids[0])
        id_end = int(matched_ids[1])

    if id_begin > id_end:
      id_begin, id_end = id_end, id_begin
