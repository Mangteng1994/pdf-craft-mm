...
class LLM:
  ...
  def _encode_markdown(self, response: str) -> str:
    for quote in self._search_quotes("markdown", response):
      return quote
    for quote in self._search_code_blocks(response):
      return quote
  ...
  def _encode_json(self, response: str) -> Any:
    ...
    for candidate in self._search_json_candidates(stripped):
      try:
  ...
  def _search_code_blocks(self, response: str) -> Generator[str, None, None]:
    start_marker = "```"
    end_marker = "```"
    start_index = 0
    ...
    while True:
      ...
      extracted_text = response[header_end + 1:end_index].strip()
      if extracted_text:
        yield extracted_text
      start_index = end_index + len(end_marker)

  def _search_json_candidates(self, response: str) -> Generator[str, None, None]:
    decoder = json.JSONDecoder()
    for i, char in enumerate(response):
      if char not in "{[":
        continue
      try:
        _, end = decoder.raw_decode(response[i:])
      except JSONDecodeError:
        continue
      yield response[i:i + end]
