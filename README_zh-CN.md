...
extractor: PDFPageExtractor = create_pdf_page_extractor(
  ...
  model_dir_path="models", # 相对于项目根目录
)
...
analyse(
  ...
  pdf_path="inputs/book.pdf", # 相对于项目根目录
  analysing_dir_path="work/analysing/book", # 分析过程的工作目录
  output_dir_path="work/output/book", # 结构化分析输出目录
)
```
...
generate_epub_file(
  ...
  epub_file_path="dist/book.epub", # 相对于项目根目录
)
```
